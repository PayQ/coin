...
// 8349 = 1*5000 + 1*1000 + 1*500 + 1*100 + 1*50 + 1*10 + 1*5 + 1
static const int ZQ_6110 = 8349;
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 ALCUP at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zALCUP send
enum ZerocoinSpendStatus {
    ZALCUP_SPEND_OKAY = 0,                            // No error
    ZALCUP_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZALCUP_WALLET_LOCKED = 2,                         // Wallet was locked
    ZALCUP_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZALCUP_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZALCUP_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZALCUP_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZALCUP_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZALCUP_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZALCUP_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZALCUP_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZALCUP_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZALCUP_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZALCUP_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZALCUP_SPENT_USED_ZALCUP = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZalcupAutoBackups(bool fEnabled)
    {
...
public:
    ...
    CAmount GetAnonymizableCredit(bool fUseCache = true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == MNA * COIN) continue; // do not count MN-like outputs
    ...
    CAmount GetLockedCredit() const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            if (fMasterNode && vout[i].nValue == MNA * COIN) {
                nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
            }

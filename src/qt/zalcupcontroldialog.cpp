...
#include "ui_zalcupcontroldialog.h"
...
std::list<std::string> ZalcupControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZalcupControlDialog::listMints;

ZalcupControlDialog::ZalcupControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZalcupControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZalcupControlDialog::~ZalcupControlDialog()
{
    delete ui;
}

void ZalcupControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZalcupControlDialog::updateList()
{
...
void ZalcupControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZalcupControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZalcup_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZalcupControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZalcupControlDialog::GetSelectedMints()
{
...
void ZalcupControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);

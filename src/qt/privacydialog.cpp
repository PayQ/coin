                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zALCUP ought to be enough for anybody." - Bill Gates, 2017
    ui->zALCUPpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzALCUPSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzALCUP->setEnabled(false);
        ui->pushButtonMintzALCUP->setToolTip(tr("zALCUP is currently disabled due to maintenance."));

        ui->pushButtonSpendzALCUP->setEnabled(false);
        ui->pushButtonSpendzALCUP->setToolTip(tr("zALCUP is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zALCUPpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzALCUP_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zALCUP is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zALCUP...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zALCUP in ") +
...
void PrivacyDialog::on_pushButtonSpendzALCUP_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zALCUP is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zALCUP
        sendzALCUP();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zALCUP
    sendzALCUP();
}

void PrivacyDialog::on_pushButtonZalcupControl_clicked()
{
    ZalcupControlDialog* zalcupControl = new ZalcupControlDialog(this);
    zalcupControl->setModel(walletModel);
    zalcupControl->exec();
}

void PrivacyDialog::setZalcupControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzalcupSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzALCUP()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid alphacup Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zALCUPpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zALCUP is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " ALCUP </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zALCUP</b>";
    ...
    // use mints from zalcup selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZalcupControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZalcupControlDialog::GetSelectedMints();
    }
    ...
    // Clear zalcup selector in case it was used
    ZalcupControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zalcup Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " alphacup, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zalcup Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zALCUP </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zALCUP "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zALCUP "));
    ui->labelzALCUPAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzALCUPSyncStatus->setVisible(fShow);
}

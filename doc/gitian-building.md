...
*Setup instructions for a gitian build of alphacup using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the alphacup
...
- [Building alphacup](#building-alphacup)
...
Clone the git repositories for alphacup and gitian and then checkout the alphacup version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/alphacupcash/alphacup-coin.git
cd alphacup
...
Follow the instructions in [doc/release-process.md](release-process.md) in the alphacup repository
...
To build alphacup (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the alphacup repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/alphacup/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/alphacupcash/alphacup-coin
...
For example:
```bash
URL=https://github.com/alphacupcash/alphacup-coin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit alphacup=${COMMIT} --url alphacup=${URL} ../alphacup/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit alphacup=${COMMIT} --url alphacup=${URL} ../alphacup/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit alphacup=${COMMIT} --url alphacup=${URL} ../alphacup/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/alphacup-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/alphacup-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/alphacup-build.assert
```
